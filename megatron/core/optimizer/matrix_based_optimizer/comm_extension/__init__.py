...
def ensure_comm_extension():
    """Prepare the native helper without loading it into this module."""
    from .build import ensure_comm_extension as _ensure_comm_extension

    return _ensure_comm_extension()


def _get_ext():
    """Build/load the native helper on first use."""
    global _ext
    if _ext is None:
        with _ext_lock:
            if _ext is None:
                from .build import load_comm_extension

                _ext = load_comm_extension()
    return _ext
...
def coalesced_allgather(outputs, input, group, async_op):
    ...
    """
    _get_ext().allgather_coalesced(outputs, input, group, async_op)
...
def coalesced_reduce_scatter(output_tensor, inputs, group, reduce_op, async_op):
    ...
    """
    _get_ext().reduce_scatter_coalesced(output_tensor, inputs, group, int(reduce_op), async_op)
